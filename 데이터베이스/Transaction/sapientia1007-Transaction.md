...
- **즉시 갱신** (Immediate Update) : 갱신데이터 → 로그, 버퍼 → DB 작업이 동시에 일어나는 것으로, 트랜잭션 실행 중 변경 내용을 즉시 데이터베이스에 반영하는 방식
- **지연 갱신** (Deferred Update) : 갱신데이터 → 로그가 끝난 후 부분완료를 하고 버퍼 → DB 작업이 진행되는 것으로, 트랜잭션이 완료되기 전까지 변경 내용을 보류하고 성공적으로 완료된 후 데이터베이스에 반영하는 방식
