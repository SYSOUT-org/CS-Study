...
다음과 같은 비정규화 상태의 데이터베이스가 있다고 가정해본다.

다음과 같은 비정규화 상태의 데이터베이스가 있다고 가정해본다. 이 상태를 각 정규화에 맞게 수정해본다.

| 학번 | 이름 | 대학 | 전공 | 과목명 | 성적 | 수강인원 |
| --- | --- | --- | --- | --- | --- | --- |
| 101 | 위진영 | IT대학 | 컴퓨터공학과 | 컴퓨터구조 | A+ | 30 |
| 101 | 위진영 | IT대학 | 컴퓨터공학과 | C언어 | A | 30 |
| 102 | 양석진 | 이과대학 | 물리학과 | JAVA | A | 30 |
| 102 | 양석진 | 이과대학 | 물리학과 | 파이썬 | B | 20 |
| 103 | 신재혁 | 공과대학 | 전자공학과 | 어셈블리 | A | 20 |
